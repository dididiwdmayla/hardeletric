/* HARD ELETRIC — feedback de arco elétrico no clique.
   ...
   Geometria: contêiner fixed no ponto do clique (clientX/clientY) com
   translate(-50%,-50%); svg com viewBox centrado na origem e todos os arcos
   partindo de 0,0. A animação usa só opacity e scale — nenhum translate. */
(function () {
  ...
  function ignorar(alvo) {
    if (!alvo || !alvo.closest) return true;
  ...
  document.addEventListener('pointerdown', function (e) {
    ...
    // Em botão primário, os arcos nascem do centro do botão e a borda brilha.
    var primario = e.target.closest && e.target.closest('a[href*="wa.me"], a[href^="tel:"], [data-arco-centro]');
    if (primario) {
      var r = primario.getBoundingClientRect();
      disparar(r.left + r.width / 2, r.top + r.height / 2);
      var antes = primario.style.boxShadow;
      primario.style.boxShadow = '0 0 0 2px var(--azul-glow, #6FC3FF)';
      setTimeout(function () { primario.style.boxShadow = antes; }, DURACAO);
      return;
    }
